...
bool production(int argc, char* argv[])
{
    ...
    displaySpace(theSpaceP, 20);

    Marker* startingMarker = placeStartingMarker(theSpaceP, 20);

    displaySpace(theSpaceP, 20);


    Marker* tempP = startingMarker;
    printf("Starting marker has been placed at (%d,%d). \n\n", tempP->col, tempP->row);
    for(int i = 0; i<20; i++)
    {
        tempP = moveMarker(theSpaceP, 20, tempP);
        displaySpace(theSpaceP, 20);
        printf("Marker has moved to  (%d,%d). \n\n", tempP->col, tempP->row);
    }
    printf("Printing total history of marker movement:\n");
    printHistoryList();
